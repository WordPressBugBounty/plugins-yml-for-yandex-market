...
if ( ! function_exists( 'wooс_delete_product' ) ) {
	...
	function wooс_delete_product( $id, $force = false ) {
		...
		if ( empty( $product ) ) {
			// translators: 1: Product type (e.g., 'product'). 2: Product ID.
			return new WP_Error( 999, sprintf( __( 'No %1$s is associated with #%2$d', 'yml-for-yandex-market' ), 'product', $id ) );
		}
		...
		if ( ! $result ) {
			// translators: 1: Product type (e.g., 'product').
			return new WP_Error( 999, sprintf( __( 'This %1$s cannot be deleted', 'yml-for-yandex-market' ), 'product' ) );
		}
