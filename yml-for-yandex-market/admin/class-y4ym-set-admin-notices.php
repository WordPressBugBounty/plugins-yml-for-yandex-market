<?php defined( 'WPINC' ) || exit;

/**
 * This class registers and outputs plugin notifications.
 *
 * @link       https://icopydoc.ru
 * @since      5.7.0
 * @version    5.7.0 (17-08-2026)
 *
 * @package    Y4YM
 * @subpackage Y4YM/admin
 */

/**
 * This class registers and outputs plugin notifications.
 * 
 * Hooked into `admin_notices` action hook.
 *
 * Usage example: `new Y4YM_Set_Admin_Notices('Logs were cleared', 'success', true);`
 * 
 * @since      5.7.0
 * @package    Y4YM
 * @subpackage Y4YM/admin
 * @author     Maxim Glazunov <icopydoc@gmail.com>
 */
class Y4YM_Set_Admin_Notices {

	/**
	 * The notice message value.
	 * @var string
	 */
	protected $message;

	/**
	 * The notice arguments.
	 * @var array
	 */
	protected $args;

	/**
	 * Initialization notice.
	 * 
	 * @param string $message Notice message value.
	 * @param string $class Notice message class. Maybe: `success`, `error`, `warning`, `info`, `(empty)`.
	 * @param bool $dismissible
	 */
	public function __construct( string $message, string $class = 'info', bool $dismissible = false ) {

		$this->message = $message;
		$this->args = [
			'type' => $class,
			'dismissible' => $dismissible
		];
		$this->init_hooks();

	}

	/**
	 * Initialization hooks.
	 * 
	 * @return void
	 */
	public function init_hooks() {

		// наш класс, вероятно, вызывается не раньше срабатывания хука admin_menu.
		// admin_init - следующий в очереди срабатывания, на хуки раньше admin_menu нет смысла вешать.
		// add_action('admin_init', [ $this, 'my_func' ], 10, 1);
		$message = $this->get_message();
		$args = $this->get_args();
		add_action( 'admin_notices', function () use ($message, $args) {
			$this->print_admin_notices( $message, $args );
		}, 10, 2 );

	}

	/**
	 * Print admin notice.
	 * @see https://wp-kama.ru/function/wp_admin_notice
	 * 
	 * @param string $message Notice message value.
	 * @param array  $args {
	 *     Optional. Arguments to control the message.
	 *
	 *     @type string $type        Notice type: 'success', 'error', 'warning', 'info'.
	 *     @type bool   $dismissible Whether the notice can be dismissed by the user.
	 * }
	 * @return void
	 */
	private function print_admin_notices( string $message, array $args ) {

		if ( function_exists( 'wp_admin_notice' ) ) {
			wp_admin_notice( $message, $args );
		} else {
			$class = 'notice';
			if ( in_array( $args['type'], [ 'success', 'error', 'warning', 'info' ] ) ) {
				$class .= ' notice-' . $args['type'];
			}
			if ( ! empty( $args['dismissible'] ) ) {
				$class .= ' is-dismissible';
			}
			printf( '<div class="%s"><p>%s</p></div>', esc_attr( $class ), esc_html( $message ) );
		}

	}

	/**
	 * Get notice message value.
	 * 
	 * @return string
	 */
	public function get_message() {
		return $this->message;
	}

	/**
	 * Get notice message arguments.
	 * 
	 * @return array
	 */
	public function get_args() {
		return $this->args;
	}

}