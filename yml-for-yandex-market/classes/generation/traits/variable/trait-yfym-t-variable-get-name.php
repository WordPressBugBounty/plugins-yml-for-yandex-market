	...
	public function get_name( $tag_name = 'name', $result_xml = '' ) {
		...
		if ( empty( $no_group_id_arr ) ) {
			...
			if ( in_array( $сur_сategory_id, $no_group_id_arr ) ) {
				$add_in_name_arr = maybe_unserialize( yfym_optionGET( 'yfym_add_in_name_arr', $this->get_feed_id() ) );
		...
		if ( $yml_rules === 'vk' ) {
			$result_yml_name_itog = trim( mb_substr( $result_yml_name_itog, 0, 99 ) ); // не более 99 символов
		}
